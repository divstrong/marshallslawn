...
<div class="cov-list">
        ...
        <div class="cov-row">
            @if ($on)
                ...
                <div class="cov-date">
                    ...
                    <div class="cov-date-d">{{ $date->format('j') }}</div>
                </div>
            @endif
            ...
            <span class="cov-row-amount">${{ number_format($job->total(), 2) }}</span>
        </div>
    @empty
    ...
    @if ($snap['completedCount'] > $snap['recentCompleted']->count())
        <p class="cov-empty">+ {{ $snap['completedCount'] - $snap['recentCompleted']->count() }} more completed</p>
    @endif
</div>
